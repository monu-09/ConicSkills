...
class DownloadFileWorkManager(
    ...
    override suspend fun doWork(): Result {
        ...
        try {
            ...
            val foregroundInfo = ForegroundInfo(NOTIFICATION_ID, showProgress(
                0,
                fileName,
                Utils.getProgressDisplayLine(total, lengthOfFile.toLong())
            ))
            ...
            val output = FileOutputStream(context.cacheDir.toString() + "/" + filePath)
            val data = ByteArray(1024)
            while (input.read(data).also { count = it } != -1) {
                total += count.toLong()
                if (lengthOfFile > 0) { // only if total length is known
                    val progress = (total * 100 / lengthOfFile).toInt()
                    showProgress(progress, fileName, Utils.getProgressDisplayLine(total, lengthOfFile.toLong()))
                }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            if (androidDownloadChannel == null) {
                androidDownloadChannel = NotificationChannel(
                    id, channelName, NotificationManager.IMPORTANCE_HIGH
                )
            }
    ...
    private fun showProgress(progress: Int, name: String, progressDisplayLine: String ): Notification {
        val id = applicationContext.getString(R.string.download_channel_id)
